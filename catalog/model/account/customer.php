...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		// NEED TO ADD CURL HERE STARTS
			$verification_code = mt_rand(1111,9999);
			$this->db->query("INSERT INTO " . DB_PREFIX . "phone_verifications SET 
				phone_number = '" . $data['country_code'].$data['telephone'] . "',
			...
			$message = "Your verification code from EmileRassam is: ".$verification_code;
			$phone_number = preg_replace("/[^0-9]/", "", $data['country_code'].$data['telephone']);
			$this->generateAndSendVerificationCode($message, $phone_number);
		// NEED TO ADD CURL HERE ENDS

		return $customer_id;
	}
	
	public function generateAndSendVerificationCode($message, $number) {
		$username = 'emilerasam';
		$password = 'Emile123';
		$msg = $message;
		$texttype = 'text';
		...
		$url = 'https://best2sms.com/http.php?username=' . urlencode($username) . '&password=' . urlencode($password) . '&msg=' . urlencode($msg) . '&texttype=' . urlencode($texttype) . '&numbers=' . urlencode($numbers) . '&sender=' . urlencode($sender);
	...
	public function editCustomer($customer_id, $data) {
		...
		$customer_old_data = $this->getCustomer($customer_id);
		if( ($customer_old_data['country_code'] != $data['country_code']) || ($customer_old_data['telephone'] != $data['country_code'].$data['telephone']) ) {
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET is_phone_verified = 0 WHERE customer_id = '" . (int)$customer_id . "'");
		}
		// die;

		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['country_code'].$data['telephone']) . "', country_code = '" . $this->db->escape($data['country_code']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "' WHERE customer_id = '" . (int)$customer_id . "'");
	}
	...
	public function getLocalizationData($country_code) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "country WHERE iso_code_2 = '" . $country_code . "'");
		return $query->row;
	}

	public function getCountryDataByCountryName($country_name) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "country WHERE name = '" . $country_name . "'");
		return $query->row;
	}
	public function savePhoneVerificationCode($customer_info) {

		$this->db->query("UPDATE " . DB_PREFIX . "phone_verifications SET status = 0 WHERE country_code = '" . $customer_info['country_code'] . "' AND  phone_number = '" . $customer_info['telephone'] . "' ");
		...
		$message = "Your verification code from EmileRassam is: ".$verification_code;
		$phone_number = preg_replace("/[^0-9]/", "", $customer_info['telephone']);
		$this->generateAndSendVerificationCode($message, $phone_number);
		
	}

	public function codeVerificationProcess($customer_info, $phone_code) {
		$get_verification_code = $this->db->query(" SELECT * FROM " . DB_PREFIX . "phone_verifications WHERE country_code = '" . $customer_info['country_code'] . "' AND  phone_number = '" . $customer_info['telephone'] . "' AND status = 1 ORDER BY phone_verifications_id DESC LIMIT 1 ");
		
		if( isset($get_verification_code->row['verification_code']) && $get_verification_code->row['verification_code'] != '' && $get_verification_code->row['verification_code'] > 0 ) {
			if( $get_verification_code->row['verification_code'] === $phone_code ) {
				return 1;
			}
		}
	}
	
	public function updateUserPhoneVerificationStatus($customer_info, $phone_code) {
		$this->db->query("UPDATE " . DB_PREFIX . "phone_verifications SET status = 0 WHERE country_code = '" . $customer_info['country_code'] . "' AND  phone_number = '" . $customer_info['telephone'] . "' ");
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET is_phone_verified = 1 where customer_id = '" . $customer_info['customer_id'] . "' ");
	}
	
	public function getCustomerByPhone($phone, $customer_id=NULL) {
		if( $customer_id != '' ) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE telephone = '" . $phone . "' AND customer_id != '".$customer_id."' ");
		} else {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE telephone = '" . $phone . "' ");
		}

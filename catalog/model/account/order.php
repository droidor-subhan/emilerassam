...
class ModelAccountOrder extends Model {
	public function cancelOrder($data) {
		...
		$language_details = $this->model_localisation_language->getLanguage($order_info['language_id']);

		$language = new Language($language_details['code']);
		$language->load($language_details['code']);
		...
		$data['order_status'] = "Cancelled by Customer";
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET `order_status_id` = '17' WHERE `order_id` = '" . (int)$data['order_id'] . "'");

		$this->db->query("INSERT INTO `" . DB_PREFIX . "order_history` SET order_id = '" . (int)$order_info['order_id'] . "', order_status_id = '17', notify = '1', comment = '" . $this->db->escape(strip_tags($data['comment'])) . "',	 date_added = NOW()");
	...
	public function getOrderCancellationMessages() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cancel_order_messages WHERE status = '1' ");
		return $query->rows;
	}

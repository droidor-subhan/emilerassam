<?php
class ModelPagesCareers extends Model {
    public function getContent($content_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "page_careers WHERE id = '".$content_id."' ");
        return $query->row;
    }
    ...
    public function saveCareers() {
        ...
        $check_query = $this->db->query("
            SELECT email 
            FROM " . DB_PREFIX . "page_careers_form 
            WHERE email = '$email'
        ");
        ...
        if ($check_query->num_rows > 0) {
            ...
            $query = $this->db->query("
            INSERT INTO " . DB_PREFIX . "page_careers_form 
            SET 
                sur_name = '$sur_name', 
                capacity = '$capacity', 
                phone = '$phone', 
            ...
            <td colspan="2" style="background-color: #000; color:#FFF; padding: 20px; text-align: center; font-size: 18px; font-weight: bold;">
                Emile Rassam - Career Request Submitted
            </td>
            </tr>
            <tr>
            ...
            $mail->setSubject(html_entity_decode("Emile Rassam - Career Request", ENT_QUOTES, 'UTF-8'));
            // $mail->setText(strip_tags("Ye Aik message hay"));

...
class ModelJournal3Filter extends Model {
	...
	public function getCategories() {
		...
		if (is_numeric($filter_category_id = Arr::get(static::$filter_data, 'filter_category_id'))) {
			...
			// if( $filter_category_id == 324 ) {
				// $sql .= " AND (c.parent_id = '" . $this->db->escape(326) . "' OR c.parent_id = '" . $this->db->escape(325) . "') ";	
			// }

			if( isset(static::$filter_data['categories']) && count(static::$filter_data['categories']) > 0 ) {

// echo '<pre>';
// 				print_r(static::$filter_data['categories']);
// 			echo '</pre>';

					$sql .= " AND (";
					foreach (static::$filter_data['categories'] as $key => $value) {
						if( $key > 0 ) {
							$sql .= " OR ";	
						}
						$sql .= " c.parent_id = '" . $this->db->escape($value) . "' ";
					}
					$sql .= " ) ";
			} else {

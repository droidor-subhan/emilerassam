...
class ModelJournal3Product extends Model {
	...
	public function getProduct($product_id) {
		...
		foreach ($query->rows as $row) {
			...
			$product_price = ($row['discount'] ? $row['discount'] : $row['price']);
			if( isset($this->session->data['market_id']) && $this->session->data['market_id'] != '' && $this->session->data['market_id'] > 0 ) {
				$get_market_price = $this->db->query("SELECT * from " . DB_PREFIX . "products_market_prices where product_id = '" . (int)$row['product_id'] . "' AND market_id = '" . (int)$this->session->data['market_id'] . "' ");

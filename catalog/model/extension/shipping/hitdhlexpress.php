...
// echo '<pre>'; print_r($weight_query);die();
			...
			$shipping_hitdhlexpress_SA_based_SS = ($this->config->get('shipping_hitdhlexpress_SA_based_SS') == 1) ? 1 : 0;
			$rate_cost_final = 0;
			if( $shipping_hitdhlexpress_SA_based_SS  == 1 && $address['country_id'] == 184 ) {
					$origin_postcode_city_for_SA = $this->hit_get_postcode_city("AE", "Abu Dhabi", "51133");
					...
					$xml .= '		</BkgDetails>';
					$xml .= '		<To>';
					$xml .= '			<CountryCode>AE</CountryCode>';
					$xml .= '			<City>abu dhabi</City>';
					$xml .= '		</To>';
					...
					if ($result && !empty($result->GetQuoteResponse->BkgDetails->QtdShp)) {
						...
						foreach ($result->GetQuoteResponse->BkgDetails->QtdShp as $quote) {
							...
							if(in_array($rate_code,$selected_services_aaray)) {
								$rate_cost_final = $this->currency->convert($rate_cost, $selected_currency, $this->config->get('config_currency'));
							}
						}
					}
			}
			...
			if ($result && !empty($result->GetQuoteResponse->BkgDetails->QtdShp)) {
				...
				foreach ($result->GetQuoteResponse->BkgDetails->QtdShp as $quote) {
					...
					$rate_cost = (float)((string) $quote->ShippingCharge) + $rate_cost_final;

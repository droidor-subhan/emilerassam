...
class ControllerJournal3ProductTabs extends ModuleController {
	...
	private function productContent($type, $short_description_limit) {
		if (static::$PRODUCT_INFO === null) {
			...
			$data['attribute_icons_list'] = $this->model_catalog_product->getProductAttributesIcons($this->request->get['product_id']);
			
			if( isset($data['attribute_groups']) && $data['attribute_groups'] != '' && is_array($data['attribute_groups']) && count($data['attribute_groups']) > 0 ) {
				foreach ($data['attribute_groups'] as $key => $attribute) {
					if( isset($attribute['attribute_group_id']) && $attribute['attribute_group_id'] == 11 ) {
						$data['attribute_groups'][$key]['icons_list'] = $data['attribute_icons_list'];
					}
					
				}
			}

			$data['base_url'] = HTTP_SERVER."/image/catalog/custom/products_icons_attributes/";
			// echo "<pre>";
			...
			static::$PRODUCT_INFO['attributes'] = $this->renderView('journal3/module/product_blocks_attributes', $data, true);

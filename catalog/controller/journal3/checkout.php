...
class ControllerJournal3Checkout extends \Journal3\Opencart\Controller {
	...
	public function index() {
		...
		$country_code = "";
		if( isset( $data['checkout_data']['order_data']['payment_country_id'] ) && $data['checkout_data']['order_data']['payment_country_id'] != '' ) {

			$query_results = $this->db->query("SELECT country_code FROM " . DB_PREFIX . "country WHERE country_id = '" . $data['checkout_data']['order_data']['payment_country_id'] . "'");
			$country_code = $query_results->row['country_code'];
		}
		...
		$data['cart_block'] = $this->renderView('journal3/checkout/cart', array(
			...
			'taxes_message'       => $this->language->get('taxes_message'),
		));
	...
	public function save() {
		...
		$data['telephone'] = Arr::get($this->request->post, 'order_data.telephone_code').$data['telephone'];
	...
	public function getSearchedCitiesZones() {
		$searched = $this->request->get['term'];
		$countr_id = $this->request->get['country_id'];
		$zones = $this->model_localisation_zone->getZonesByCountryIdAndSearchedKeyWord($countr_id, $searched);
		$result = array();
		foreach ($zones as $zone) {
			$result[] = array(
				'id' => $zone['zone_id'],

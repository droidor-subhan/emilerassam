...
class ControllerPaymentPayfortFortQpay extends Controller {
    ...
    public function index() {
        ...
        if (version_compare(VERSION, '2.2.0.0') >= 0) {
            $this->template = 'extension/payment/payfort_fort_qpay';
        }
        else{
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/payfort_fort_qpay')) {
                $this->template = $this->config->get('config_template') . '/template/extension/payment/payfort_fort_qpay';
            } else {
                $this->template = 'default/template/extension/payment/payfort_fort_qpay';
            }
        }

...
class ControllerAccountEdit extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['country_code'])) {
			...
			if( $data['country_code'] != '' ) {
				$data['telephone'] = str_replace($data['country_code'], '', $data['telephone']);
			}
		...
		$countries_list = $this->model_localisation_country->getCountries();
	...
	protected function validate() {
		...
		if ( (!is_numeric($this->request->post['telephone'])) || $this->request->post['telephone'] <= 0 ) {
			$this->error['telephone'] = "Phone number must be numeric and greater than 0";
		}

		if (!isset($this->request->post['country_code']) || $this->request->post['country_code'] == "" ) {
			$this->error['error_country_code'] = "please select Country Code";
		}
		...
		if( isset($session_data['customer_id']) && $session_data['customer_id'] != '' && $session_data['customer_id'] > 0 ) {
			...
			$this->load->model('account/customer');
			$getCustomerByPhone = $this->model_account_customer->getCustomerByPhone( ($this->request->post['country_code'].$this->request->post['telephone']), $session_data['customer_id'] );
			if( $getCustomerByPhone > 0 ) {
				$this->error['telephone'] = "Phone number already Exists.";
			}

		} else {
			$this->error['telephone'] = "No User found.";
		}

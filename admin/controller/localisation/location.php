...
class ControllerLocalisationLocation extends Controller {
	...
	public function markets() {
		$this->load->language('localisation/market');
		$this->document->setTitle($this->language->get('heading_title'));
	...
	public function addMarkets() {
		...
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model('localisation/market');
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateFormMarkets()) {
			
			$this->model_localisation_market->addMarket($this->request->post);
			...
			$this->response->redirect($this->url->link('localisation/location/markets', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	public function editMarkets() {
		$this->load->language('localisation/market');
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model('localisation/market');
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateFormMarkets()) {
			$this->model_localisation_market->editLocation($this->request->get['markets_id'], $this->request->post);
	...
	public function deleteMarkets() {
		...
		if (isset($this->request->post['selected'])) {
			foreach ($this->request->post['selected'] as $location_id) {
				$this->model_localisation_market->deleteMarkets($location_id);
			}
	...
	protected function getMarketListing() {
		if (isset($this->request->get['sort'])) {
			$sort = $this->request->get['sort'];
		} else {
			$sort = 'name';
		...
		$data['breadcrumbs'][] =   array(
			'text' =>  $this->language->get('heading_title'),
			'href' =>  $this->url->link('localisation/location/markets', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('localisation/location/addMarkets', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('localisation/location/deleteMarkets', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$location_total = $this->model_localisation_market->getTotalMarkets();

		$results = $this->model_localisation_market->getMarkets($filter_data);

		foreach ($results as $result) {
			$data['markets'][] =   array(
				'markets_id' => $result['markets_id'],
				'code'        => $result['code'],
				'currency_name'        => $result['title'],
				'status'        => $result['status'],
				'edit'        => $this->url->link('localisation/location/editMarkets', 'user_token=' . $this->session->data['user_token'] . '&markets_id=' . $result['markets_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/location/markets', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url, true);
		$data['sort_address'] = $this->url->link('localisation/location/markets', 'user_token=' . $this->session->data['user_token'] . '&sort=address' . $url, true);
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('localisation/location/markets', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('localisation/markets_list', $data));
	}

	
	protected function getFormMarkets() {
		$data['text_form'] = !isset($this->request->get['markets_id']) ? $this->language->get('text_add') : $this->language->get('text_edit');
		...
		if (!isset($this->request->get['markets_id'])) {
			$data['action'] = $this->url->link('localisation/location/addMarkets', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('localisation/location/editMarkets', 'user_token=' . $this->session->data['user_token'] .  '&markets_id=' . $this->request->get['markets_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('localisation/location/markets', 'user_token=' . $this->session->data['user_token'] . $url, true);
		if (isset($this->request->get['markets_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$location_info = $this->model_localisation_market->getLocation($this->request->get['markets_id']);
		}
		...
		if (isset($this->request->post['name'])) {
			...
			$data['name'] = $location_info['code'];
		} else {
			$data['name'] =   '';
		}


		if (isset($this->request->post['curency'])) {
			$data['curency'] = $this->request->post['curency'];
		} elseif (!empty($location_info)) {
			$data['curency'] = $location_info['currency_id'];
		} else {
			$data['curency'] = '';
		}
		...
		$getCurrencies = $this->model_localisation_currency->getCurrencies();
		$data['currencies'] = $getCurrencies;
		$this->response->setOutput($this->load->view('localisation/market_form', $data));
	}
	...
	protected function validateFormMarkets() {

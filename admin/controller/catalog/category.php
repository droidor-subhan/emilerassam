...
class ControllerCatalogCategory extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['path_left'])) {
			$data['path_left'] = $this->request->post['path_left'];
		} elseif (isset($category_info['image_1_category_id']) && $category_info['image_1_category_id'] != '' ) {
			$category_details = $this->model_catalog_category->getCategoryDescriptionsEnglish($category_info['image_1_category_id']);
			if( isset($category_details['name']) ) {
				$data['path_left'] = $category_details['name'];
			} else {
				$data['path_left'] = "";
			}
		...
		if (isset($this->request->post['path_right'])) {
			$data['path_right'] = $this->request->post['path_right'];
		} elseif (isset($category_info['image_2_category_id']) && $category_info['image_2_category_id'] != '' ) {
			$category_details = $this->model_catalog_category->getCategoryDescriptionsEnglish($category_info['image_2_category_id']);
			if( isset($category_details['name']) ) {
				$data['path_right'] = $category_details['name'];
			} else {
				$data['path_right'] = "";
			}
		} else {
		...
		if (isset($this->request->post['parent_id_left'])) {
			$data['parent_id_left'] = $this->request->post['parent_id_left'];
		} elseif (!empty($category_info)) {
			$data['parent_id_left'] = $category_info['image_1_category_id'];
		} else {
			$data['parent_id_left'] = 0;
		}

		if (isset($this->request->post['parent_id_right'])) {
			$data['parent_id_right'] = $this->request->post['parent_id_right'];
		} elseif (!empty($category_info)) {
			$data['parent_id_right'] = $category_info['image_2_category_id'];
		} else {
			$data['parent_id_right'] = 0;
		}

		// Category Size Guide
		if( isset($this->request->get['category_id']) && $this->request->get['category_id'] != '' ) {
			$category_size_guide = $this->model_catalog_category->getCategoryPdf($this->request->get['category_id']);
			if( isset($category_size_guide[0]['media']) && $category_size_guide[0]['media'] !=''  ) {
				$data['size_guide_icon_image'] = HTTP_CATALOG . "image/catalog/custom/no_image_backup.png";
				$data['size_guide'] = HTTP_CATALOG . "image/catalog/custom/category_size_guides/".$category_size_guide[0]['media'];
			} else {
		...
		// Category image Left & Right Side
		$data['category_image_one'] = "";
		$data['category_image_two'] = "";
		if( isset($this->request->get['category_id']) && $this->request->get['category_id'] != '' ) {
			$category_size_guide = $this->model_catalog_category->getCategoryImages($this->request->get['category_id']);
			if( isset($category_size_guide[0]['category_image_one']) && $category_size_guide[0]['category_image_one'] !=''  ) {
				$data['category_image_one'] = HTTP_CATALOG . "image/catalog/custom/category_images/".$category_size_guide[0]['category_image_one'];
			}
			if( isset($category_size_guide[0]['category_image_two']) && $category_size_guide[0]['category_image_two'] !=''  ) {
				$data['category_image_two'] = HTTP_CATALOG . "image/catalog/custom/category_images/".$category_size_guide[0]['category_image_two'];
			}
		}
